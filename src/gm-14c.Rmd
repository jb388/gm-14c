...
I started with 5000 iterations, but some of the parameters did not stabilize within that number of runs. The data from the second run, with 15000 is slightly better, but there are still issues with parameter convergence (see plots below). Owing to the long run time of these calculations, I have split the MCMC code into a separate script. The output of that script can then be loaded back into this file to generate the report.

```{r load-bayes-fits}
# the following .RData files are generated by script "/Users/jeff/gm-14c/src/gm-14c-mcmc-bayes.R"
load(file = "../data/derived/bayes-par-fit-2020-05-15/bayes.s.3p.15000itr.RData")
load(file = "../data/derived/bayes-par-fit-2020-05-15/bayes.z.3p.15000itr.RData")


## look at model performance
# Sohi
bayesFacts.s <- data.frame(niter, burnin, bayes_fit_s_3p$naccepted)
colnames(bayesFacts.s) <- c('# Iterations', "# Burn-In", "# Accepted")
# Zimmerman
...
From the above plots, we can see that some of the parameters still haven't stabilized after 15000 iterations (note that the first 5000 iterations were discarded, "burnin"). The transfer coefficients seem particularly unstable. This could be due to the initial values, but is more likely because the the model is underconstrained.
