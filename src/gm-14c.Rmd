...
ages <- seq(1, 4000) # Time vector for density functions
...
The above table shows the mean ages and transit times estimated from the manually fit models. The Sohi model has a substantially older mean age because of the higher amount of carbon partitioned to the slowest cycling pool. Interstingly the transit times are very similar, likely because all of the inputs flow through the fastest pool, which dominates the respiration flux. 
...
sa.pool.dist.df %>%
  ...
  ggtitle("Intermediate pool age distributions") +
  theme_bw() +
  theme(panel.grid = element_blank())
...
Vertical lines in the above plots show the mean ages for each pool and model. In this case the intermediate aged pool is the most similar between the two models, while the fast and slow pools look quite different.
...
# look at estimates
s.mod.fit$par
s.pars.i # much higher k value for fPOM, much higher transfer coefficient to oPOM, lower to Omin
...
# look at estimates
z.mod.fit$par
z.pars.i # much higher k value for fast and lower for intm pools, much higher t coef to oPOM, lower to Omin
...
# summarize parameter sets
s.3p.opt.par <- data.frame(par = c("kfPOM", "koPOM", "kOmin", "fPOM to oPOM", "fPOM to Omin"),
                           value = round(s.mod.fit$par, 5))

knitr::kable(s.3p.opt.par,
             caption = "Optimized parameters: Sohi 3-pool model",
...
ggplot(s.3ps.C14.df, aes(years, d14C, color = pool)) +
  geom_path() +
  geom_path(data = s.3ps.C14.df.fit, linetype = "dashed") +
  geom_point(aes(2011, ws.14c.nobc), color = "black", size = 3) + 
  scale_color_manual(
    name = "Model pool",
    values = c("atm" = 8,
...
ggplot(z.3ps.C14.df, aes(years, d14C, color = pool)) +
  geom_path() +
  geom_path(data = z.3ps.C14.df.fit, linetype = "dashed") +
  geom_point(aes(2011, ws.14c.nobc), color = "black", size = 3) + 
  scale_color_manual(
    name = "Model pool",
    values = c("atm" = 8,
...
niter <- 5000 # start low (time consuming)
burnin <- 500 # low rejection rate first time (Shane used 1/3)
...
end - start # 4 min w/ 1000 iterations
